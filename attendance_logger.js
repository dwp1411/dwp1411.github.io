...
const TARGET_COL_FORTHILL_INDEX = 55; // Column BC (1-based for getRange)
...
function runAttendanceLog(supervisorName) {
  ...
  if (jobColMap.size === 0) {
    throw new Error(`CRITICAL: No Job Headers found in Row 3 of the Timecard tab "${dayTabName}". Has the Timecard file been modified or is Row 3 empty?`);
  }
...
function processSupervisor(sourceSheet, targetSheet, jobColMap, supervisorName) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Explicitly skip the header row to prevent shifting everyone down by 1 row
    ...
    // ALWAYS write Name to Column B, regardless of Present status.
    ...
    if (present === 'Y') {
      ...
      // Ultra-strict normalization: remove ALL spaces, newlines, and punctuation for bulletproof matching
      const normalizedJob1 = job1.replace(/[^a-z0-9]/gi, '').toLowerCase();
      const targetCol1 = jobColMap.get(normalizedJob1);
      ...
      if (row.length > COL_JOB_2) {
        ...
        if (job2) {
          const normalizedJob2 = job2.replace(/[^a-z0-9]/gi, '').toLowerCase();
          const targetCol2 = jobColMap.get(normalizedJob2);
          if (targetCol2) {
            let hours2 = 0;
      ...
      // For absent employees (not 'Y'), log 0 hours for their primary job
      const job1 = String(row[COL_JOB_1]).trim();
      if (job1) {
        const normalizedJob1 = job1.replace(/[^a-z0-9]/gi, '').toLowerCase();
        const targetCol1 = jobColMap.get(normalizedJob1);
        if (targetCol1) {
          addUpdate(updates, currentTargetRow, targetCol1, 0);
        }
      }
    }

    // Increment row pointer for the next employee (present or absent)
    currentTargetRow++;
  }
  ...
  // 4. ALERT UNMAPPED JOBS
  if (unmappedJobs.size > 0) {
    const unmappedArr = Array.from(unmappedJobs);
    const msg = `WARNING: The following Job Functions from the Tracker could not be found in the Timecard headers for today:\n\n- ${unmappedArr.join('\n- ')}\n\nPlease ensure the spelling in the Tracker dropdown exactly matches the Timecard header for today.`;
...
/**
 * Builds a map for Job Functions (Column index) from the target sheet (Row 4).
...
function buildJobHeadersMap(targetSheet) {
  ...
  // Job functions are in Row 4 (Index 3)
  if (targetData.length > 3) {
    const headerRow = targetData[3];
    // Start scanning from Index 8 (Column I) to protect columns C through H
    for (let c = 8; c < headerRow.length; c++) {
      const job = String(headerRow[c]).trim();
      if (job) {
        const normalizedJob = job.replace(/[^a-z0-9]/gi, '').toLowerCase();
        // Only set the FIRST occurrence of a job header. Do not overwrite if it appears twice.
        if (!jobColMap.has(normalizedJob)) {
          jobColMap.set(normalizedJob, c + 1); // 1-based column index
        }
      }
    }
  }
